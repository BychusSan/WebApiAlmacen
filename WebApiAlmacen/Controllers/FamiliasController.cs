...
namespace WebApiAlmacen.Controllers
{
    ...
    public class FamiliasController : ControllerBase
    {
        ...
        public FamiliasController(MiAlmacenContext context, OperacionesService operacionesService, ILogger<FamiliasController> logger)
        {
            _context = context;
        ...
        public async Task<List<Familia>> GetFamilias()
        {
            ...
            // Si en el appsettings.Development.json marcamos un nivel de log superior al information, por ejemplo Warning, los mensajes
            // de información no se van a ver
            _logger.LogWarning("OBTENIENDO FAMILIAS");
            _logger.LogError("OBTENIENDO FAMILIAS");
            ...
            //_logger.LogInformation($"PRUEBA PRUEBA PRUEBA PRUEBA {lista}");
            return lista;
            
        }

        // Si en la ruta ponemos una / con un texto, para ejecutar este endopoint, deberemos poner en la url localhost:xxxxx/familias
        // Si ponemos [HttpGet("familias")] la ruta parte de api/familias, luego el endpoint sería ejecutado bajo la url localhost:xxxxx/api/familias
        [HttpGet("/familias")]
